...
function contextHtml(c,max=180){
  ...
  return \`<button type="button" class="context-preview w-full" onclick="openConversation('\${c.id}')"><div class="text-xs opacity-50">\${esc(label)} · click to view conversation</div><div>\${esc(compact)}\${!hasAi&&c.opening&&c.opening.text.length>max?'…':''}</div></button>\`;
}
