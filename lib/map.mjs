...
const TIDY_STAFF_IDENTIFIERS = new Set(
    [
        ...
        "Mathew Nicholls", "Michelle Nicol", "Monica Shepherd", "Nathan Travis", "Peter", "Peter Clapcott", "Phillip Dong",
...
// A few historic CRM messages expose only a first name. Keep these out of the
// general staff list: they are staff only when the CRM also marks the message
// as support-originated, so a customer sharing the name remains a customer.
const SUPPORT_FLAGGED_STAFF_ALIASES = new Set(["peter"]);
export function isTidyStaffMessage(message) {
    return isKnownTidyStaff(message?.sender) || (message?.fromSupport === true && SUPPORT_FLAGGED_STAFF_ALIASES.has(normalizeName(message?.sender)));
}
...
export function mapChat(chat, messages) {
    ...
    const lastIsFromStaff = mappedLast && !mappedLast.isNote && isTidyStaffMessage(mappedLast);
