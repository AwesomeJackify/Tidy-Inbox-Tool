...
import { detectAiBackend, parseCustomArgs } from "./ai-backend.mjs";
...
function runCommand(command, args, prompt, source, unwrap = (stdout) => stdout) {
...
export async function runAiJson(prompt, { model = process.env.TIDY_AI_MODEL, preferred } = {}) {
    ...
    if (ai.backend === "codex") {
        ...
        return { value: await runCommand("codex", args, prompt, "Codex"), source: ai.source };
    }

    if (ai.backend === "claude") {
        const args = ["-p", "--output-format", "json", ...(model ? ["--model", model] : [])];
        const value = await runCommand("claude", args, prompt, "Claude Code", (stdout) => JSON.parse(stdout).result ?? "");
        return { value, source: ai.source };
    }

    const args = parseCustomArgs(process.env.TIDY_AI_ARGS).map((arg) => arg.replaceAll("{model}", model || ""));
    return { value: await runCommand(process.env.TIDY_AI_COMMAND, args, prompt, process.env.TIDY_AI_NAME || "Custom AI"), source: ai.source };
}
